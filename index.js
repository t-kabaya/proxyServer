...
const onErr = (err, msg, url, soc) => {
  if (soc) soc.end()
  console.log('Error: %s %s: %s', new Date().toLocaleTimeString(), msg, url, err + '')
}

const logRequestInfos = (cliReq) => {
    console.log('------------------------------')
    console.log(cliReq.method)
    console.log('url: ', cliReq.url)
    console.log('header: ', cliReq.headers)
